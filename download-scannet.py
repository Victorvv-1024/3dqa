...
def download_release(release_scans, out_dir, file_types, use_v1_sens, skip_existing):
    ...
    # Define a helper function for downloading a single scan
    def download_single_scan(scan_id):
        scan_out_dir = os.path.join(out_dir, scan_id)
        download_scan(scan_id, scan_out_dir, file_types, use_v1_sens, skip_existing)

    # Use ThreadPoolExecutor for parallel downloads
    with ThreadPoolExecutor(max_workers=56) as executor:  # Adjust max_workers based on your system
        executor.map(download_single_scan, release_scans)
