...
class PointViewFusion(BaseModule):
    ...
    1.  **Decoupled Projection for Attention:** Projects raw point and view features
        to a common `hidden_dim` ONLY for calculating attention scores.
    2.  **Point-Guided View Aggregation:** Uses projected point features as a
        query to aggregate the multi-view features with manual dot-product attention.
    3.  **Pure PID Synergy Extraction:** Uses shared mathematically correct synergy extractor.
    4.  **Final Projection:** Projects to target fusion dimension.
    """
