...
# embodiedqa/models/framework/point_view_fusion.py - Updated
...
class PointViewFusion(nn.Module):
    """
    Simplified Point-View Fusion without superpoint dependencies.
    Uses attention-based fusion with implicit geometric awareness.
    """
    ...
    def __init__(self, point_dim, view_dim, fusion_dim, hidden_dim=256):
        ...
        # Attention mechanism for point-guided view selection
    ...
    def forward(self, point_features, view_features, superpoint_ids=None):
        ...
        Args:
            point_features: Point cloud features [B, Np, D_point]
            view_features: View features [B, Np, D_view] (already aggregated from multi-view)
        ...
        # Use point features to attend to view features
        attended_view, attention_weights = self.attention(
            query=point_proj,     # Points guide the attention
            key=view_proj,        # View features as keys
            value=view_proj       # View features as values
        )
        
        # Combine point and attended view features
