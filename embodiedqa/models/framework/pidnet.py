...
class PIDNet(BaseModel):
    ...
    """
    _version = 2 # Version 1 is Early Fusion of PID components, Version 2 is Late Fusion
    def __init__(self,
             backbone: ConfigType,
        ...
        #     nn.Linear(self.decomposed_dim, self.decomposed_dim // 2),
        #     nn.GELU(),
        #     nn.Linear(self.decomposed_dim // 2, self.fusion_encoder.config.hidden_size)
        # )
        # self.full_visual_feat_map = nn.Sequential(
        #     nn.Linear(self.decomposed_dim, self.decomposed_dim // 2),
        #     nn.GELU(),
        #     nn.Linear(self.decomposed_dim // 2, self.fusion_encoder.config.hidden_size)
        # )
